  ...
  if ")" in p:
    while ")" in p:
      ...
      if p[startindex-1] in op:
        p = p.replace(p[startindex - 1:endindex], str(doFunc(p[startindex-1], p[startindex:endindex]))) 
      else:
        p = p.replace(p[startindex:endindex], str(solve(p[startindex:endindex]))) 
  return p 
